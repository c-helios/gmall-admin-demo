...
let allRoutes = []
allRoutes = allRoutes.concat(constantRoutes)
if (process.env.VUE_APP_ENABLE_DYNAMIC_ROUTER !== 'true') {
  allRoutes = allRoutes.concat(asyncRoutes).concat([anyRoutes])
}
...
export function resetRouter(routes) {
  const newRoutes = routes && routes.length > 0 ? routes : allRoutes
